...
function HomeScreen() {
  ...
  const login = async () => {
    try {
      ...
      uiConsole('Firebase login success', loginRes?.user?.uid);
      const idToken = await loginRes?.user.getIdToken(true);
      uiConsole('idToken obtained');
      ...
      uiConsole('Login error:', e.message);
    }
  };
  // IMP END - Login
  ...
  if (!isConnected) {
    return (
      ...
      <View style={styles.loginArea}>
        <Button
          title={connectLoading ? 'Logging in...' : 'Login with Firebase'}
          onPress={login}
        />
      </View>
      // IMP END - Login
